...
class EventProcessor {
  ...
  // IMPORTANT: Source data is already in EST, so we preserve the date as-is
  // For date-only strings (YYYY-MM-DD), we parse them directly without timezone conversion
  parseDate(dateStr) {
    if (!dateStr) return null;
    
    // Handle already parsed dates - return as-is since source is already in EST
    ...
    // Source data is already in EST, so we keep it as a string to preserve the date
    // This avoids timezone conversion issues when the Date is serialized to JSON
    const dateOnlyPattern = /^(\d{4})-(\d{2})-(\d{2})$/;
    const dateOnlyMatch = cleanDate.match(dateOnlyPattern);
    
    if (dateOnlyMatch) {
      // Return the date string as-is since source is already in EST
      // This will be sent to frontend as a string, preserving the original EST date
      return cleanDate; // Return as string, not Date object
    }
    ...
    for (let format of formats) {
      ...
      if (parsed.isValid()) {
        // Source data is already in EST, so return the parsed date as-is
